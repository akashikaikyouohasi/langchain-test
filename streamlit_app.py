...
def run_agent(input_data):
    ...
    # LangGraphのinterrupt機能を使うためにthread_idが必要。同じスレッドIDを使うことで、状態を保持できる。
    config = {"configurable": 
    ...
    with st.spinner("処理中...", show_time=True):
        for chunk in agent.stream(input_data, stream_mode="updates", config=config): 
            #stream_modeは、"updates"か"values"、"messages"などが選べる。"updates"は部分的な更新を含むチャンクを返す。
            for task_name, result in chunk.items(): # chunkは辞書型. items()で(task_name, result)のペアを取得
                # interruptの場合
                if task_name == "__interrupt__":
                    ...
                    st.session_state.waiting_for_approval = True # 承認待ちフラグを立てる
                # 最終回答の場合
        ...
        st.rerun() # Streamlitアプリを即座に再実行する関数。st.rerun()がないと、session_stateを更新しても次のユーザー操作まで画面に反映されません。
    ...
    if st.session_state.waiting_for_approval \
       and st.session_state.tool_info:
        ...
        if feedback_result:
            ...
            run_agent(Command(resume=feedback_result)) # interruptの再開コマンドを送信。resumeは再開の意味で、interruptへの戻り値となる。
            st.rerun() #Streamlitアプリを即座に再実行する関数

    # 最終結果の表示
    if st.session_state.final_result \
       and not st.session_state.waiting_for_approval:
        st.subheader("最終結果")
        st.success(st.session_state.final_result) # 成功メッセージを緑色のボックスで表示する
